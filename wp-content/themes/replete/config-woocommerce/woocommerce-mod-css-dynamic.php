...
if(!function_exists('avia_woo_dynamic_css'))
{
	...
	function avia_woo_dynamic_css($output, $color_set)
	{
		...
		foreach ($color_set as $key => $colors) // iterates over the color sets: usually $key is either: header_color, main_color, footer_color, socket_color
		{
			...
			$key .woocommerce-tabs .tabs .active, $key div.product .woocommerce-tabs .panel, $key .activeslideThumb, $key #payment li, $key .widget_price_filter .ui-slider-horizontal .ui-slider-range,  $key .avia_cart, $key form.login, $key form.register, $key .col-1, $key .col-2, $key .variations_form, $key .thumbnail_container .rating_container, $key .inner_cart_button, $key .dynamic-title, $key .single-product-main-image .thumbnails a , $key .quantity input.qty, $key .single-product-main-image .images a, $key .woocommerce-MyAccount-navigation-link.is-active, $key .woocommerce-MyAccount-navigation-link:hover{
