...
Version: 4.02
Author: Adeel Ahmed
Author URI: https://www.ip-finder.me/
*/
...
function ip_added()
{
	...
	if(filter_var($IP, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4))
	{
		...
		$IP_in_DP=$wpdb->get_var($wpdb->prepare("SELECT id FROM $table WHERE IP=%s",$IP));
...
<?php
	...
	echo "<div id=\"IPBLC_message_blacklist\">$IP added to blacklist. Please comment on <a href=\"https://www.ip-finder.me/$IP/\" target=\"_blank\">IP-FINDER.ME</a></div>";
...
{  
	...
	$rr=$wpdb->get_results("SELECT * FROM $table1 t1 WHERE NOT EXISTS (SELECT 1 FROM $table2 t2 WHERE t1.IP = t2.IP) LIMIT 50");
	$old=$wpdb->get_results($wpdb->prepare("SELECT * FROM $table2 WHERE timestamp<=%d && lastvisit<=%d LIMIT 500",array($days_90,$days_90)));
    ...
    if($newFailed==50)
    {
      $newFailed="50+";
		...
		$countMain='<span class="awaiting-mod count-'.count($rr).'"><span class="pending-count">'.$newFailed.'</span></span>';
	}
    ...
    if($count_old==500)
    {
      $count_old="500+";
		...
		$countOld='<span class="awaiting-mod count-'.count($old).'"><span class="pending-count">'.$count_old.'</span></span>';
...
function IPBLC_IP_value( $column, $comment_ID )
{
	...
	if ( 'IPBLC_IP' == $column )
	{
		...
		echo '<a href="https://www.ip-finder.me/'.$IP.'/" target="_blank" title="IP Details on IP-Finder.me">'."$IP</a>";

		echo '<div class="row-actions">
	<span class="edit">
		<a href="https://www.ip-finder.me/'.$IP.'/" target="_blank" title="IP Details on IP-Finder.me">IP Details</a>
	</span> | 
	<span class="edit">';
...
function IPBLC_blockip()
{
    ...
    $data = array('test' => '1');
    $user_agent_custom=md5(site_url());
    $contextData = array ( 
                'method' => 'POST',
...
$link="https://www.ip-finder.me/lastUpdate.php?website=".urlencode(site_url());
			...
			if($IPBLC_cloud_password && $IPBLC_cloud_on==2 && $IPBLC_cloud_password==$pwd)
			{
				...
				if($resultX)
				{
					foreach($resultX as $IPData)
					{
						...
						$link="<a href=\"https://www.ip-finder.me/$IP/\" target=_blank>$IP</a>";
						$failedID=$IPData->id;
			...
			if(current_user_can( 'manage_options' ))
			{
				...
				if(filter_var($IP, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4))
				{
					...
					if($isIPSafe==0)
					{
						...
						{
							echo "<b style=\"color: #FF0000;\"> Blacklisted</b>
								<a href=\"https://www.ip-finder.me/$IP/\" target=_blank title=\"Leave Comment\">Why?</a>";
						}
						else
						{
							echo "<b style=\"color: #009900;\"> Neutral</b>";
...
add_action( 'admin_notices', 'check_my_whitelist'); 
		...
		if($failedTotal)
		{
			...
			if($attempts>=$IPBLC_failedlogin_max && filter_var($visitorIP, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4))
			{
				$table2=$wpdb->prefix."IPBLC_blacklist";
					...
					$IP_in_DP=$wpdb->get_var($wpdb->prepare("SELECT id FROM $table2 WHERE IP=%s",$visitorIP));
					if(!$IP_in_DP)
					{
						//---check if IP is valid IPV4 or not before adding to database---
						if(filter_var($visitorIP, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4))
						{
							...
							$wpdb->query($wpdb->prepare("INSERT INTO $table2 (IP,timestamp,visits,lastvisit) VALUES(%s,%d,%d,%d)",
								array($visitorIP,$time,0,0)));
...
function protected_comment_link()
{
	...
	echo "Protected with <a href=\"https://www.ip-finder.me\"><img src=\"".plugins_url()."/ip-blacklist-cloud/icon.png\" style=\"display: inline;\" alt=\"IP Blacklist Cloud\"></a><a href=\"https://www.ip-finder.me\" title=\"IP Blacklist Cloud\">IP Blacklist Cloud</a>";


	}
	else
	{
		//echo "<div style=\"display: none;\">Protected with <a href=\"https://www.ip-finder.me\" title=\"IP Blacklist Cloud\">IP Blacklist Cloud</a></div>";
	}
}
...
function post_blacklist_add($IP)
{
    ...
    $context = stream_context_create (array ( 'http' => $contextData ));
    $link="https://www.ip-finder.me/blacklist_add.php?IP=".$IP."&website=".urlencode(site_url())."&website_name=".urlencode(get_bloginfo('name'));
...
function post_blacklist_remove($IP)
{
    ...
    $context = stream_context_create (array ( 'http' => $contextData ));
    $link="https://www.ip-finder.me/blacklist_delete.php?IP=".$IP."&website=".urlencode(site_url())."&website_name=".urlencode(get_bloginfo('name'));
...
function post_blacklist_add_multi($IPs)
{
  ...
  if($IPs)
  {
      ...
      $context = stream_context_create (array ( 'http' => $contextData ));
      $link="https://www.ip-finder.me/blacklist_add_multi.php?IP=".$IP_var."&website=".urlencode(site_url())."&website_name=".urlencode(get_bloginfo('name'));
...
function post_blacklist_remove_multi($IPs)
{
  ...
  if($IPs)
  {
      ...
      $context = stream_context_create (array ( 'http' => $contextData ));
      $link="https://www.ip-finder.me/blacklist_delete_multiple.php?IP=".$IP_var."&website=".urlencode(site_url())."&website_name=".urlencode(get_bloginfo('name'));
...
function post_blacklist_add_user($user)
{
    ...
    $link="https://www.ip-finder.me/blacklist_user_add.php?USER=".$user."&website=".urlencode(site_url())."&website_name=".urlencode(get_bloginfo('name'));
...
function post_blacklist_remove_user($user)
{
    $data = array('test' => '1');
    $user_agent_custom=md5(site_url());
    $contextData = array ( 
    ...
    $link="https://www.ip-finder.me/blacklist_delete_user.php?USER=".$user."&website=".urlencode(site_url())."&website_name=".urlencode(get_bloginfo('name'));
...
{
  //update_option('IPBLC_fixes','');
  global $wpdb;
	$IPBLC_fix=get_option('IPBLC_fixes');

	if($IPBLC_fix!=2)
	{
    include "auto_fix.php";
		update_option('IPBLC_fixes','2');
	}

}
function check_my_whitelist()
{
    $my_ip_now="";
    $is_myip_safe=1;
    ...
    if(isset($_SERVER['REMOTE_ADDR']))
    {
      $my_ip_now=$_SERVER['REMOTE_ADDR'];
      $is_myip_safe=isIpSafe($my_ip_now);
    }
    ...
    if($is_myip_safe==0 && is_admin() && !isset($_GET['addMyIp']) && $current_page!="wp-IPBLC-whitelist")
    {
    
?>
	<div class="error">
  <p>
		<b><span style="color: #FF0000;">IP Blacklist Cloud</span>: Your IP <b style="color: #009900;"><?php echo $my_ip_now; ?></b> is not added to whitelist, please <a href="admin.php?page=wp-IPBLC-whitelist&addMyIp=<?php echo $my_ip_now; ?>">add IP to whitelist</a>.</b>
  </p>  
	</div>
<?php
    }
...
function ipblc_notice()
{
	global $pagenow;
	$msg_array=array();
	$msg_array[]='Manage multiple WordPress sites with <a href="https://www.ip-finder.me/ipblc-server/">IPBLC Cloud Server</a> premium tool.';
	$msg_array[]='Tired of blocking same IP on multiple sites? <a href="https://www.ip-finder.me/ipblc-server/">IPBLC Cloud Server</a> is a solution.';
	$msg_array[]='<a href="https://www.ip-finder.me/ipblc-server/">IPBLC Cloud Server</a> - Synchronize IP addresses on multiple sites.';
